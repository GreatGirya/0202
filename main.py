...
while run:
    for e in pygame.event.get():
        if e.type == pygame.QUIT:
            run = False
        # выход из программы при нажатии на клавищу ESCAPE
            ...
            # печатает только цифры
                ...
                if int(numeral) > 100:
                    ...
                    dialogs("",OUTSIDE_BG,'Бери меньше жмот')
                elif int(numeral) < num:
                    dialogs("",OUTSIDE_BG,'Чё так мало')
                if move == 1:
                    if int(numeral) == num:
                        dialogs(f'Это число{numeral}', dialog_cat_pos,'Кiт, ты выиграл')
                        block=1
                    else:
                        dialogs('Пёс твой ход', dialog_cat_pos,'Продолжаем')
                elif move == 2:
                    if int(numeral) == num:
                        dialogs(f'Это число{numeral}', dialog_dog_pos,'Пёс, ты выиграл')
                        block=1
                    else:
                        dialogs('Кiт твой ход', dialog_dog_pos,'Продолжаем')
                numeral = ''
                move += 1
                if move > 2:
                    move = 1
